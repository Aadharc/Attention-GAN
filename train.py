...
def main():
    ...
    train_dataset = CustomDataSet(config.TRAIN_DIR_VIS, config.TRAIN_DIR_IR, config.TRAIN_DIR_VIS_lbl, config.TRAIN_DIR_IR_lbl, transform= transform)
    ...
    val_dataset = CustomDataSet(config.VAL_DIR_VIS, config.VAL_DIR_IR, config.VAL_DIR_VIS_lbl, config.VAL_DIR_IR_lbl, transform)
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        if config.SAVE_MODEL and epoch % 2 == 0:
            ...
            save_some_examples(conv_block, cross_attn, gen, val_loader, epoch, folder="resizeIRclamplabel")
